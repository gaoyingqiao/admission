<template>
  <div id="swiper" :style="{height: imgHeight + 'px'}" @mouseover="stopAuto" @mouseout="autoPlay">
    <div class="swiper-box" :style="{width:allCount,'-webkit-transition':transitionConfig,'-webkit-transform':slateX}"
         ref='swiper'>
      <swiper-item :img="imgList[imgList.length-1].slider_pic" :intro="imgList[imgList.length-1].slider_intro" :scrollWidth="-imgWidth"
                   :imgWidth="imgWidth" :imgHeight="imgHeight" v-if="loop"/>
      <swiper-item v-for="(item,index) in imgList" :key="index" :img="item.slider_pic" :intro="item.slider_intro"
                   :scrollWidth="imgWidth*index" :imgWidth="imgWidth" :imgHeight="imgHeight"/>
      <swiper-item :img="imgList[0].slider_pic" :intro="imgList[0].slider_intro" :scrollWidth="imgWidth*imgList.length"
                   :imgWidth="imgWidth" :imgHeight="imgHeight" v-if="loop"/>
    </div>
    <span class='swiper-left' @click="toLeft">←</span>
    <span class='swiper-right' @click='toRight'>→</span>
    <div class="swiper-dots" v-if="dots">
      <button v-for="(item,index) in imgList.length" :class="{active:index==dotsIndex}" @click="toDots(index)">
        {{item}}
      </button>
    </div>
  </div>
